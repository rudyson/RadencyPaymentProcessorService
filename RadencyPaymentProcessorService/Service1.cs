	...
	public partial class Service1 : ServiceBase
	{
		...
		protected override void OnStart(string[] args)
		{
			...
			midnightLogSaver = new Timer(
				SaveLogs, null,
				DateTime.Now.Date.AddDays(1) - DateTime.Now - TimeSpan.FromSeconds(10),
				TimeSpan.FromDays(1));
		...
		{
			// Saving logs
			SaveMetaLog(String.Join("/", GetTodaysDirectoryPath(), "meta.log"), true);
			SaveErrorLog(String.Join("/", GetTodaysDirectoryPath(), "error.log"), true);
		}
		...
		private void SaveErrorLog(string path, bool reset = false)
		{
			if (reset)
			{
				this.logger.Clear();
